    ...
    >
      <v-card-text>
        <div style="display: flex; justify-content: space-between;">
          <div>
            {{ $t('dialogs.cabs.unarchive.loadingText') }}
          </div>
          <div>
  ...
  created() {
    if (this.cabs.selected && Array.isArray(this.cabs.selected) && this.cabs.selected.length > 0) {
      this.cabs.selected.forEach(async cab => {
        ...
        await this.api.post('/cabs/unarchive', data)
          .catch(() => true);
        this.done++;
        if (this.cabs.selected.length === this.done) {
          this.$store.dispatch('cabs/closeDialog', 'unarchive');
