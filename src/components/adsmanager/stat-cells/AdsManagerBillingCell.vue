<template>
  <td style="font-size: 11px; padding-right: 16px !important;">
    <!-- КАРТА И БИЛЛИНГ -->
    <div style="display: flex; justify-content: space-between;">
      ...
      <div>
        {{
          cab.balance ?
            numeral(cab.balance).format('0,0.[00]').replace(',', ' ') : 0
        }}
        /
        {{
          cab.billing ?
            numeral(cab.billing).format('0,0.[00]').replace(',', ' ') : 0
        }}
        {{ cab.currency }}
      </div>
    </div>
    ...
    <div style="display: flex; justify-content: space-between;">
      <div>
        {{ $t('common.totalSpend') }}: {{ numeral(cab.spent).format('0,0.[00]').replace(',', ' ') }} {{ cab.currency }}
      </div>
      <div>
        {{ $t('common.limit') }}: 
        <span v-if="!cab.cab_limit || cab.cab_limit === -1 || cab.cab_limit === 0">{{ $t('common.no').toLowerCase() }}</span>
        <span v-else>{{ numeral(cab.cab_limit).format('0,0.[00]').replace(',', ' ') }} {{ cab.currency }}</span>
      </div>
    </div>
  </td>
</template>
