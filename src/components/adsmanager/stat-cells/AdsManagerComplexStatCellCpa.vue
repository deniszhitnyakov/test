  ...
  <td>
    ...
    >
      <span v-if="col === 'action_results'">
        {{ 
          itIsTotalRow ? 
            numeral(stat['total']['cpa']).format('0,0.[00]').replace(',', ' ') : 
            numeral(stat[item.id]['cpa']).format('0,0.[00]').replace(',', ' ') 
        }}
      </span>
      <span v-else>
        {{ 
          itIsTotalRow ? 
            numeral(stat['total']['actions'][col]['cpa']).format('0,0.[00]').replace(',', ' ') : 
            numeral(stat[item.id]['actions'][col]['cpa']).format('0,0.[00]').replace(',', ' ') 
        }}
      </span>
    </span>
    <span v-else>
